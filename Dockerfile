...
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
    CMD wget --no-verbose --tries=1 --spider http://localhost:8108/api/v1/health 2>/dev/null || \
        (apk add --no-cache wget >/dev/null 2>&1 && wget --no-verbose --tries=1 --spider http://localhost:8108/api/v1/health) || \
        exit 1
